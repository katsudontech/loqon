...
import { removePracticeBoundary, splitPracticePart, TimelineConflictError, validatePracticeParts, type CompositionCue, type PracticePart } from '@/lib/timeline'
...
export function PartsEditorContainer({ projectId, audioUrl, pdfUrl, cues, initialParts, initialVersion, initialUpdatedAt = null }: Props) {
  ...
  useEffect(() => {
    if (audio.duration <= 0 || parts.length !== 1 || parts[0].startTime !== 0 || parts[0].endTime > 0) return
    const timer = window.setTimeout(() => setParts([{ ...parts[0], endTime: audio.duration }]), 0)
    return () => window.clearTimeout(timer)
  ...
  const addBoundary = (time = audio.currentTime) => {
    if (!Number.isFinite(time) || time <= 0 || parts.some((part) => part.startTime === time || part.endTime === time)) { setError('同じ時刻の境界は追加できません'); return }
    try { update(splitPracticePart(parts, time)) } catch (cause) { setError(cause instanceof Error ? cause.message : 'パートを分けられません') }
  }
  ...
  return <div className="editor-layout relative">
    ...
    <aside className="editor-sidebar" data-expanded="true">
      <div className="timeline-head"><span>パート分け</span><span className="text-xs text-zinc-500">{parts.length}件</span></div>
      <p className="p-3 text-xs text-zinc-400">構成のページ切り替えとは別に、練習する区間を設定します。同じページ内でも分けられます。</p>
      <div className="timeline-rail mx-3" role="img" aria-label="構成キューとパート境界のタイムライン"><div className="h-2 rounded bg-amber-400/40" />{cues.map((cue) => <span key={`cue-${cue.id}`} data-kind="composition-cue" className="timeline-rail-cue" style={{ left: `${Math.min(100, Math.max(0, cue.time / Math.max(audio.duration, 1) * 100))}%` }} aria-label={`構成キュー ${cue.time.toFixed(1)}秒`} />)}{parts.slice(1).map((part) => <span key={`boundary-${part.id}`} data-kind="practice-boundary" className="timeline-rail-boundary" style={{ left: `${Math.min(100, Math.max(0, part.startTime / Math.max(audio.duration, 1) * 100))}%` }} aria-label={`パート境界 ${part.startTime.toFixed(1)}秒`} />)}</div>
      <div className="timeline-list">
        ...
        {parts.map((part) => <div key={part.id} className="timeline-item border-l-2 border-amber-400">
          <div className="timeline-item-top"><span>{part.startTime.toFixed(1)}s – {part.endTime.toFixed(1)}s</span><span className="flex gap-2">{parts.indexOf(part) > 0 && <button type="button" className="button-quiet" aria-label="この境界を削除して前後を結合" onClick={() => update(removePracticeBoundary(parts, part.id))}>境界を削除</button>}</span></div>
          <input aria-label="パート名" value={part.name ?? ''} onChange={(event) => update(parts.map((item) => item.id === part.id ? { ...item, name: event.target.value } : item))} />
          {parts.indexOf(part) > 0 && <input aria-label="境界時刻" type="range" min={parts[parts.indexOf(part) - 1].startTime + 0.01} max={part.endTime - 0.01} step="0.01" value={part.startTime} onChange={(event) => { const time = Number(event.target.value); const index = parts.indexOf(part); update(parts.map((item, itemIndex) => itemIndex === index - 1 ? { ...item, endTime: time } : itemIndex === index ? { ...item, startTime: time } : item)) }} />}
          <p className="text-xs text-zinc-500 mt-1">含むPDF: ページ{pageRange(part).start}–{pageRange(part).end}</p>
        </div>)}
      </div>
    </aside>
    <div className="editor-actions"><AudioControls {...audio} /><div className="editor-action-row"><button type="button" className="editor-action accent" onClick={() => addBoundary()}>ここでパートを分ける</button><button type="button" className="editor-action" onClick={() => setHistory((old) => { const previous = old.at(-1); if (previous) { setParts(previous); return old.slice(0, -1) } return old })} disabled={!history.length}>元に戻す</button></div>{error && <p role="alert" className="alert alert-error">{error}</p>}{message && <p role="status" className="alert alert-success">{message}</p>}<button type="button" className="button w-full" disabled={saving} onClick={() => save()}>{saving ? '保存中...' : '保存して練習'}</button><button type="button" className="button-quiet w-full" onClick={() => navigateTo(`/${projectId}/edit`)}>構成編集へ戻る</button></div>
  </div>
}
