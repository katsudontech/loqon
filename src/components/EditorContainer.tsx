    ...
    return (
        <div className="flex flex-col w-full h-full overflow-hidden relative">
                ...
                <div className="absolute inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
                    <div className="bg-zinc-900 border border-indigo-500/50 rounded-xl p-5 max-w-sm w-full">
                        ...
                        <div className="flex gap-2">
                            <button className="flex-1 bg-indigo-600 text-white rounded-lg py-2" onClick={() => { restoreMarkers(draftCandidate.markers); setDraftOpen(false) }}>復元</button>
                            <button className="flex-1 bg-zinc-700 text-white rounded-lg py-2" onClick={() => { try { window.localStorage.removeItem(timelineDraftKey(projectId)) } catch { /* optional */ } setDraftOpen(false); setDraftCandidate(null) }}>破棄</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="absolute inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
                    <div className="bg-zinc-900 border border-amber-500/50 rounded-xl p-5 max-w-sm w-full">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <button className="flex-1 min-w-[30%] bg-indigo-600 text-white rounded-lg py-2" onClick={reloadRemote}>再読み込み</button>
                            <button className="flex-1 min-w-[30%] bg-amber-600 text-white rounded-lg py-2" onClick={() => handleSaveButton(true)}>上書き</button>
                            <button className="flex-1 min-w-[30%] bg-zinc-700 text-white rounded-lg py-2" onClick={() => setConflictOpen(false)}>キャンセル</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="w-full shrink-0 bg-zinc-900 border-b border-zinc-800 flex flex-col z-20 shadow-md">
                <button onClick={() => setIsTimelineExpanded(!isTimelineExpanded)} className="flex justify-between items-center p-3 sm:p-4 w-full hover:bg-zinc-800/50 transition-colors">
                ...
                {isTimelineExpanded && <div className="w-full bg-zinc-950 flex flex-col max-h-[40vh] border-t border-zinc-800">
                    <div className="flex justify-end items-center p-2 px-4 border-b border-zinc-800 bg-zinc-900 shrink-0"><button onClick={clearMarkers} className="text-xs px-2 py-1 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded transition-colors">全削除</button></div>
                    <div className="flex-1 overflow-y-auto p-2 space-y-1">
                        {markers.length === 0 ? <div className="text-center py-6 text-zinc-500 text-xs">まだ区切りがありません。</div> : [...markers].sort((a, b) => a.time - b.time).map((m, i, arr) => {
                            ...
                            return <div key={m.id} className="flex flex-col gap-2 bg-zinc-800 rounded px-3 py-2 border border-zinc-700/50"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="font-mono text-xs sm:text-sm text-indigo-300 bg-indigo-500/10 px-2 py-0.5 rounded">{m.time.toFixed(1)}s</span><span className="text-zinc-300 font-medium text-xs sm:text-sm">Page {m.page} {partNumber > 1 && <span className="text-zinc-500 text-[10px] sm:text-xs ml-1">Part {partNumber}</span>}</span></div><button onClick={() => deleteMarker(m.id)} className="text-red-400 hover:text-red-300 p-1 rounded"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1 1v3M4 7h16" /></svg></button></div><input type="text" maxLength={100} value={m.name || ''} onChange={(e) => updateMarkerName(m.id, e.target.value)} placeholder="フォーメーション名（任意）" className="w-full bg-zinc-900 border border-zinc-700 rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-indigo-500 transition-colors" /></div>
                        })}
                    </div>
                </div>}
            </div>
            ...
            <div className="w-full shrink-0 flex flex-col bg-zinc-950 border-t border-zinc-800 pb-safe">
                ...
                <div className="p-3 sm:p-4 flex flex-col gap-2 shrink-0 bg-zinc-950">
                    <div className="flex gap-2"><button onClick={handleRecordPartChange} className="flex-1 py-3 sm:py-4 bg-zinc-800 hover:bg-zinc-700 text-white font-bold rounded-xl shadow-lg border border-zinc-700 transition-all active:scale-[0.98] text-sm sm:text-base">📍 パート区切り</button><button onClick={handleRecordPageTurn} disabled={numPages !== null && currentPage >= numPages} className="flex-1 py-3 sm:py-4 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-bold rounded-xl shadow-lg transition-all active:scale-[0.98] disabled:from-zinc-600 disabled:to-zinc-700 disabled:shadow-none disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base">📄 次のページへ</button></div>
                    ...
                    <button onClick={() => handleSaveButton()} disabled={isSaving} className="w-full py-3 mt-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm sm:text-base rounded-xl shadow-lg transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"><span>💾</span> {isSaving ? '保存中...' : '保存してプレイヤーへ'}</button>
                </div>
            </div>
        </div>
    )
